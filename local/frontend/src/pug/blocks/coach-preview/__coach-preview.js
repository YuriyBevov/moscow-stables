...
// код с расстановко колонок !! Рабочий!!!
document.addEventListener("DOMContentLoaded", () => {
	initGalleryColumns();
	positionEvenColumns(); // Добавляем эту строку
	initAnimations();
});
...
function initGalleryColumns() {
	...
	document.querySelectorAll(".gallery-grid-column").forEach((column) => {
...
function positionEvenColumns() {
	...
	const evenColumns = document.querySelectorAll(".gallery-column--even");
	...
	evenColumns.forEach((column) => {
		...
		// Сохраняем параметры
		column.dataset.initialY = offset;
		column.dataset.baseHeight = column.scrollHeight;
	});
}
